...
class CorrectnessScoreTests(unittest.TestCase):
    ...
    def test_worst_correctness(self):
        mock = MockCorrectnessScore(stargazers_count=0, open_issues_count=0, forks_count=0)
        sut = CorrectnessScore(mock)
        score = sut.score()
        self.assertEqual(score, 0)

    def test_best_correctness(self):
        mock = MockCorrectnessScore(stargazers_count=10000, open_issues_count=400, forks_count=10000)
        sut = CorrectnessScore(mock)
        score = sut.score()
